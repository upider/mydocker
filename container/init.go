...
/*
这里的init函数是在容器内部执行的，也就是说代码执行到这里后容器所在进程其实就已经创建出来了，
这是本容器执行的第1个进程。
使用mount先去挂载proc文件系统，以便后面通过ps等系统命令去查看当前进程资源情况。
*/
...
func setUpMount() {
	...
	//mount proc
	log.Infof("mount proc and tmpfs")
...
func pivotRoot(root string) error {
	...
	*/
	...
	//函数原型PivotRoot(newroot string, putold string) (err error)
